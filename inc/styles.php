...
function alzira_hex2rgba($color, $opacity = false) {
...
//Dynamic styles
function alzira_custom_styles($custom) {
	...
	if ( (get_theme_mod('front_header_type','slider') == 'nothing' && is_front_page()) || (get_theme_mod('site_header_type') == 'nothing' && !is_front_page()) ) {
		...
		$rgba 	= alzira_hex2rgba($menu_bg_color, 0.9);
	...
	wp_add_inline_style( 'alzira-style', $custom );	
}
add_action( 'wp_enqueue_scripts', 'alzira_custom_styles' );