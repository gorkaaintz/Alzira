...
;(function($) {
   ...
   'use strict'
		...
		function sliderHeight() {
			...
			$('#slideshow').imagesLoaded( function() {
				if ( $(window).width() <= 1024 ){	
					...
					$('.alzira-hero-area, #slideshow').height(slideItemHeight);
				} else {
					$('.alzira-hero-area').css('height', 'auto');
				}
			});
		}
	...
	var videoPopup = function() {
		...
		function closePopup() {
			if ( $('.alzira-video.vid-lightbox .video-overlay').hasClass('popup-show') ) {
			    
				var popup = $('.alzira-video.vid-lightbox .video-overlay.popup-show');
		...
		$('.alzira-video.vid-lightbox .video-overlay').on('click',function () {
			closePopup();
		});

		$('.alzira-video.vid-lightbox').parents('.panel-row-style').css({'z-index': '12', 'overflow': 'visible'});	
