...
class Alzira_Portfolio extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'alzira_portfolio_widget', 'description' => __( 'Display your projects in a grid.', 'alzira') );
       parent::__construct(false, $name = __('Alzira FP: Portfolio', 'alzira'), $widget_ops);
		$this->alt_option_name = 'alzira_portfolio_widget';
  }
  ...
  public function widget( $args, $instance ) {
    ...
    $options = array(
     ...
     'include'       => $includes,
     'filter'        => $show_filter,
     'show_all_text' => ! empty( $show_all_text ) ? $show_all_text : __('Show all', 'alzira')
    );
       ...
       if ( has_post_thumbnail() ) {
           ...
           if ( $project_url ) :
               $output .= '<div class="project-item item isotope-item ' . $termsString . '">';
							 ...
							 $output .= get_the_post_thumbnail($post->ID,'alzira-mas-thumb');
							 $output .= '</a>';
							 $output .= '</div>';
           else :
               $output .= '<div class="project-item item isotope-item ' . $termsString . '">';
 ...
 function form($instance) {
    ...
    $show_all_text = isset( $instance['show_all_text'] )  ? esc_html($instance['show_all_text']) : __('Show all', 'alzira');

	?>

   <p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'alzira'); ?></label>
   <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
   <p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of projects to show (-1 shows all of them):', 'alzira' ); ?></label>
   <input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
   <p><label for="<?php echo $this->get_field_id('includes'); ?>"><?php _e('Enter the slugs (comma separated) for your categories or leave empty to show all projects.', 'alzira'); ?></label>
   ...
   <label for="<?php echo $this->get_field_id( 'show_filter' ); ?>"><?php _e( 'Show navigation filter? (Category slugs must be specified).', 'alzira' ); ?></label></p>
	 <p><label for="<?php echo $this->get_field_id('show_all_text'); ?>"><?php _e('"Show all" text:', 'alzira'); ?></label>
   ...
   <label for="<?php echo $this->get_field_id( 'show_project_title' ); ?>"><?php _e( 'Show project title?', 'alzira' ); ?></label></p>
